...
local globalkeys = gears.table.join(
	...
	-- File Shortcuts
	awful.key({ modkey }, "g", function() awful.spawn("thunar Downloads/") end,
		{ description = "Open file manager at ~/Downloads/", group = "file manager" }),
...
-- Goes to next not-empty tag
function next_tag()
	local index = nil

	local first_before_index = nil
	for i, tag in ipairs(awful.screen.focused().tags) do
		if #tag:clients() > 0 and index ~= nil then
			awful.tag.viewidx(i - index, awful.screen.focused())
			return
		elseif #tag:clients() > 0 and first_before_index == nil then
	...
	awful.tag.viewidx(first_before_index - index, awful.screen.focused())
end

-- Goest to previous non-empty tag
